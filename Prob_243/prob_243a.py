...
n = 1
pfactor = fractions.Fraction(1,1)
for p in primes:
    # Calculate answer for n = 2^i * 3 * 5 * ... * pk
    i = 2
    while (2**i < p):
        nn = n * 2**i
        result = pfactor * fractions.Fraction(nn,nn-1)
        print "    * 2^{0}: pfactor = {1} = {2}, result = {3} = {4}, nn = {5}".format(i,float(pfactor),pfactor,float(result),result,nn)
        if (result < target):
            print "Answer = {0}".format(nn)
            break
        i += 1

    # Calculate answer for n = 2 * 3 * 5 * ... * pk
    n *= p
    pfactor *= fractions.Fraction(p-1,p)
    result = pfactor * fractions.Fraction(n,n-1)
    print "* {0}: pfactor = {1} = {2}, result = {3} = {4}, n = {5}".format(p,float(pfactor),pfactor,float(result),result,n)
    if (result < target):
