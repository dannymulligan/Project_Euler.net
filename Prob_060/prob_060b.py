...
spaces = search_limit - 4
print "Will skip {0} of these primes in any set we try".format(spaces)

top = 4 + spaces
for a in range(0,top-3):
    pa = str(primes[a])

    for b in range(a+1,top-2):
        pb = str(primes[b])
        ...
        for c in range(b+1,top-1):
            ...
            for d in range(c+1,top):
                ...
                print "Found {0} (sum = {1})".format([primes[a], primes[b], primes[c], primes[d]], (primes[a] + primes[b] + primes[c] + primes[d]))
                exit()
