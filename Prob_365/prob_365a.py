...
def func_m(n,k,m):
    ...
    for i in xrange(r):
        ...
        while ((num % m) == 0):
            num = num / m
            print "  ...dividing numerator by {0}".format(m)
        while ((den % m) == 0):
            den = den / m
            print "  ...dividing denominator by {0}".format(m)
...
for ri in range(2,len(c_n_k_m)):
    ...
    for qi in range(1,ri):
        ...
        for pi in range(0,qi):
            ...
            print "M(10^{0},10^{1},{2}) = {3}, ".format(POW_n, POW_k, pqr[pi], c_n_k_m[pi]),
            print "M(10^{0},10^{1},{2}) = {3}, ".format(POW_n, POW_k, pqr[qi], c_n_k_m[qi]),
            print "M(10^{0},10^{1},{2}) = {3} => ".format(POW_n, POW_k, pqr[ri], c_n_k_m[ri]),
            print "M(10^{0},10^{1},{2}) = {3} ".format(POW_n, POW_k, pqr[pi]*pqr[qi]*pqr[ri], x)
...
b = binomial(n,k);  print "binomial({0},{1}) = {2}".format(n,k,b)
for m in trym:
    e = (b % m);  print "binomial({0},{1}) % {2} = {3}".format(n,k,m,e)
