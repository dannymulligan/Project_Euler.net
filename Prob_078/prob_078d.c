...
long ways(int n, int max_n)
{
    ...
    for(i = 1; i <= max_n; i++) {
        ...
        } else if ((x<CACHE_SIZE) && (y<CACHE_SIZE) && (ways_cache[x][y] != 0)) {
        ...
        //
        // when i = max_n, we have calculated ways(n,max_n)
        //   so we store this value in ways_cache2[i]
        if ((i==max_n) && (i<CACHE_SIZE)) {
            // Save the result in ways_cache2[i]
            ways_cache2[i] = ans;
            fprintf(fp_res, "ways(%d,%d) = %d\n", i, i, ans);
            if (debug)  printf("depth=%d ", iteration_d);
            if (debug)  printf("XX    ways_cache2[%d] <=write= %d\n", i, ans);
        } else if ((n<CACHE_SIZE) && (i<CACHE_SIZE)) {
...
void init_cache () {
    ...
    for (n=0; n<CACHE_SIZE; n++) {
        ways_cache2[n] = 0;
        if (debug)  printf("    ways_cache2[%d] <=write= %d\n", n, 0);
