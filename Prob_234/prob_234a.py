...
for i in xrange(len(primes)-1):
    ...
    # add all of the numbers between l_limit & u_limit that are multiples of l_prime
    l_n = (l_limit-1) / l_prime
    u_n = u_limit / l_prime
    answer += l_prime * ((u_n+1)*u_n/2 - (l_n+1)*l_n/2)
    #print "      prime={0} l_n={1} u_n={2}".format(l_prime, l_n, u_n)
    #print "  Answer =", answer

    # add all of the numbers between l_limit & u_limit that are multiples of u_prime
    l_n = (l_limit-1) / u_prime
    u_n = u_limit / u_prime
    answer += u_prime * ((u_n+1)*u_n/2 - (l_n+1)*l_n/2)
    #print "      prime={0} l_n={1} u_n={2}".format(u_prime, l_n, u_n)
    #print "  Answer =", answer

    # subtract all of the numbers between l_limit & u_limit that are multiples of l_prime*u_prime
    l_n = (l_limit-1) / (l_prime*u_prime)
    u_n = u_limit / (l_prime*u_prime)
    answer -= 2 * (l_prime*u_prime) * ((u_n+1)*u_n/2 - (l_n+1)*l_n/2)
    #print "      prime={0} l_n={1} u_n={2}".format(l_prime*u_prime, l_n, u_n)
    #print "  Answer =", answer
