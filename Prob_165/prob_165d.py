...
def intersect_point(x0,y0, x1,y1, x2,y2, x3,y3):
    ...
    if ((x0-x1)*(x2-x3) != 0):  # No vertical lines
        ...
        # (y-y0) = (x2-x0)*(y1-y0)/(x1-x0)
        # y = y0 + (x2-x0)*(y1-y0)/(x1-x0)
        # y*(x1-x0) = y0*(x1-x0) + (x2-x0)*(y1-y0)
        #
        #     y0*(x1-x0) + (x2-x0)*(y1-y0)
        # y = ----------------------------
        #               (x1-x0)
        y_top = y0*(x1-x0) + (x2-x0)*(y1-y0)
        y_bot = (x1-x0)
        dem = gcd(y_top,y_bot)
        fy = (y_top/dem, y_bot/dem)
        ...
        # Then given that we know x, we can calculate y
        # (y-y2) = (x0-x2)*(y3-y2)/(x3-x2)
        # y = y2 + (x0-x2)*(y3-y2)/(x3-x2)
        # y*(x3-x2) = y2*(x3-x2) + (x0-x2)*(y3-y2)
        #
        #     y2*(x3-x2) + (x0-x2)*(y3-y2)
        # y = ----------------------------
        #               (x3-x2)
        y_top = y2*(x3-x2) + (x0-x2)*(y3-y2)
        y_bot = (x3-x2)
        dem = gcd(y_top,y_bot)
        fy = (y_top/dem, y_bot/dem)
