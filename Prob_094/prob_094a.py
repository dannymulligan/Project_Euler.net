...
len = a + b + c
s = (a + b + c) / 2
area = int((s * (s - a) * (s - b) * (s - c))**.5)
print "{0}, l = {1}, a = {2}".format((a, b, c), len, area)
total = 0
if (len < 1000000000):
    total += len

while (len < LIMIT):
    (a, b, c) = (2*a+3*c, 2*a+3*c, 2*c+a)
    len = (a + b + c)
    s = (a + b + c) / 2
    area = int((s * (s - a) * (s - b) * (s - c))**.5)
    area_f = (s * (s - a) * (s - b) * (s - c))**.5
    print "{0}, l = {1}, a = {2}, a = {3}".format((a, b, c), len, area, area_f)
    if (len < 1000000000):
        total += len
